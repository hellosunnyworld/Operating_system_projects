...
__device__ void user_program(FileSystem *fs, uchar *input, uchar *output) {
  ...
  //show(fs,fp);
	fs_read(fs, output, 32, fp); // input + 32, 32
  //show(fs,fp);
	fs_gsys(fs,LS_D);
	fs_gsys(fs, LS_S);
	fp = fs_open(fs, "b.txt\0", G_WRITE);
  ...
  /* u32 crtAddr;
   ...
   u32 fp = fs_open(fs, fname[0], G_WRITE);
   show(fs,fp);
   fs_write(fs, input + 1024 * 0, 64, fp);
   show(fs,fp);
   for (int i = 1; i < 1024; i++) {
    ...
    //printf("addTimes=%d\n",addTimes);  
    fs_write(fs, input + 1024 * i, 1024, fp);
    //crtAddr = checkStartingAddr(fs,fp);
    //if ((crtAddr - (i*1024+64-1024) - 36*1024) > 0)
      //printf("overfow at %d!\n", i);
      //printf("crt:    %x\n", crtAddr);
      //printf("normal: %x\n\n\n", (i*1024+64-1024) + 36*1024);
    show(fs,fp);
   }
   //printf("fname[0] = %s\n", fname[0]);
   //fs_gsys(fs, RM, fname[0]);
   //printf("fname[0] = %s\n", fname[0]);
   fp = fs_open(fs, fname[0], G_WRITE);
   //printf("fname[0] = %s\n", fname[0]);
   show(fs,fp);
   fs_write(fs, input, 1024, fp);
   show(fs,fp);
   fs_gsys(fs, LS_S);*/
/*
  // Test case4: to test the fs_read function
   u32 fp;
   char fname2[1018][20];
   int p = 0;
