...
__device__ u32 crtPathFp;
__device__ int fileNum;
__device__ bool dirt;
							...
							int MAX_FILE_NUM, int MAX_FILE_SIZE, int FILE_BASE_ADDRESS)
{
  ...
  gtime = 0;
  dirt = 0;
  crtPathFp = fs->SUPERBLOCK_SIZE;
  fs_gsys(fs, MKDIR, "root\0");
  crtPathFp = search_FCB_entry(fs, "root\0", MKDIR); //root
}
...
__device__ u32 search_FCB_entry(FileSystem *fs, char* s, int op){
  ...
  for (int i = 0; i < fs->FCB_ENTRIES; i++){
    ...
    //if ((nullNum == 20)){ 
      ...
      //modifyParent(fs, baseAddr, crtPathFp);
      //printf("after add\n");
      return baseAddr;
    }
    nullNum = 0;
...
__device__ u32 checkParent(FileSystem *fs, u32 fp){
  uint32_t low = fs->volume[fp + 30] >> 6;
  uint32_t high = fs->volume[fp + 29];
  return 4*1024+(((uint32_t)(high << 2) + (uint32_t)low) >> 5); 
}

// p is the address in volume of parent fp
__device__ void modifyParent(FileSystem *fs, u32 fp, u32 p){
  p = ((p-4*1024) << 17) >> 22; // 10-bit fp
  uint8_t low = (p << 30) >> 24; // low000000
  fs->volume[p + 30] |= low;
  uint8_t high = fs->volume[fp + 29] >> 2;
  fs->volume[p + 29] = high;
}
...
__device__ u32 fs_write(FileSystem *fs, uchar* input, u32 size, u32 fp)
{
  ...
  if (dirt){
    size = size - fsize;
    uchar content[50][20];
    u32 num = retrieveContent(fs, crtPathFp, content);
    for (u32 i = 0; i < size; i++){
      content[num][i] = input[i];
    }
    int j, k;
    k = 0;
...
__device__ void fs_gsys(FileSystem *fs, int op)
{
  if ((op == LS_S) || (op==LS_D)){
    ...
    uchar content[50][20];
    int num = retrieveContent(fs, crtPathFp, content);
    for (u32 i = 0; i < num; i++){
      records[i] = search_FCB_entry(fs, (char*)content[i], op);
      records[i] = fp;
      //printf("item %d: %s\n", 0, records[0]->name);
    ...
    crtPathFp = checkParent(fs, crtPathFp);
  }
}
// Return the number of file names in given string uchar content[50][20]
__device__ int retrieveContent(FileSystem *fs, u32 fp, uchar content[50][20]){
  ...
  while (1){
    ...
    while (fs->volume[addr+i] != '\0'){
      content[j][k] = fs->volume[addr+i];
      i++;
      k++;
    }
    content[j][k] = fs->volume[addr+i];
    i++;
    if (fs->volume[addr+i] == '\0')
      break;
  }
  return (j+1);
}

__device__ bool checkSubDir(FileSystem *fs, uchar* s){
  uchar content[50][20];
  u32 num = retrieveContent(fs, crtPathFp, content);
  u32 size = checkSize(fs, crtPathFp);
  int j = 0;
  int k = 0;
  for (int i = 0; i < num; i++){
    k = 0;
    while ((content[i][j] == s[k]) && (content[i][j] != '\0') && (s[k] != '\0')){
      j++;
      k++;
    }
    if (content[i][j] != s[k])
...
__device__ void fs_gsys(FileSystem *fs, int op, char *s)
{
  if (op == RM){
    ...
    dirt = 1;
    // Write to parent
    fs_write(fs, (uchar*)s, fsize+k, crtPathFp);
    dirt = 0;
  }
  else if (op == CD){
    if (checkSubDir(fs, s))
      crtPathFp = search_FCB_entry(fs, s, op);
    else
...
__device__ void show(FileSystem *fs, u32 fp){
      ...
      for (int i = 0; i < 5; i++){
        for (int j = 0; j < 32; j++){
          name[0] = fs->volume[4*1024+32*i+j];
          printf("%s",name);
        }
        printf("\n");
      }
