...
extern __device__ __managed__ int pagefault_num = 0;
...
int main() {
  ...
  //write data.bin (same size as disk storage) into input buffer 
  int input_size = load_binaryFile(DATAFILE, input, STORAGE_SIZE); 

  /* Launch kernel function in GPU, with 4 threads
  ...
  //dump the content of result buffer to snapshot.bin
